...
import { pcpLoading, pcpViewMode, pcpInit, pcpNextInit, pcpUpdateSingleItem, pcpGetBanner } from './reducer';
...
const pcpBannerAction = ({ token, query }) => async (dispatch, getState) => {
	dispatch(pcpLoading({ isLoading: true }));
	...
	const path = `${baseUrl}/categories/banner`;
	...
	if (err) {
		dispatch(pcpLoading({ isLoading: false }));
		return Promise.reject(__x(err));
	}

	const pcpBanner = response.data.data;
	dispatch(pcpGetBanner({ pcpBanner }));

	return Promise.resolve(pcpBanner);
};
