	...
	function Picker(props) {
		...
		_this.scrollHanders = function () {
			...
			var onWheel = function onWheel(e) {
				...
				wheeler += 1;
				scrollY = e.deltaY < 0 ? scrollY - (wheeler * 34) : (wheeler * 34) + scrollY;
				var height = (_this.props.children.length - 1) * _this.itemHeight;

				if (scrollY % _this.itemHeight !== 0) {
					scrollY = Math.round(scrollY / _this.itemHeight) * _this.itemHeight;
				}
				if (scrollY < 0) {
					scrollY = 0;
	...
	_createClass(Picker, [{
		...
		value: function componentDidMount() {
			...
			Object.keys(this.scrollHanders).forEach(function (key) {
				if (key.indexOf('touch') === 0 || key.indexOf('mouse') === 0 || key.indexOf('wheel') === 0) {
